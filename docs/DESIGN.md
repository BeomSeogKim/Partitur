...
A CLI-originated proposal records `requires_decision: false`: no attempt is waiting on it.
Routing nevertheless opens its human decision. On a CLI route, `amend` reports the allocated
proposal and decision ids, routed reason, and actual impact; it also reports the envelope evaluation
when policy evaluated it. The operator may inspect those computed routing facts before invoking
`approve`. Its pending decision is non-blocking: it does not put an attempt, movement, or run in
`WAITING_HUMAN`.
...
amendment.approved {              # the SINGLE authoritative transition (§9)
  ...
  typed_delta: [{selector, operation, before_hash?, after_hash?}],
  actual_impact,
  head_movements: [{              # score-derived lifecycle projection (§9), in declaration order
    id,
    initial: PENDING | INAPPLICABLE,
    repo_write: bool,
    has_dependencies: bool,
    final: bool
  }],                             # exact projection of the new snapshot, not a score duplicate:
                                  # it is the minimum replay needs to reconcile Movements,
