...
func TestMutationCrossEdgeSemanticRecovery(t *testing.T) {
	...
	for _, mutation := range []struct {
		...
		{
			name:      "checkpoint-derived charge formula is checked",
			before:    "\tif charged != want {\n",
			after:     "\tif false && charged != want { // mutation\n",
			target:    "TestSemanticRecoveryNormalizer/checkpoint_derived_charge_formula_no_checkpoint",
			signature: "invalid no-checkpoint charge error=<nil>",
		},
		{
			name:      "checkpoint grace is added to the baseline charge",
			before:    "\twant := checkpointMS + runstate.AccountingGraceMS\n",
			after:     "\twant := checkpointMS // mutation\n",
			target:    "TestSemanticRecoveryNormalizer/checkpoint_derived_charge_formula_no_checkpoint",
			signature: "valid no-checkpoint charge: checkpoint-derived charged_duration=35000",
		},
		{
			name:      "latest checkpoint baseline is read into the charge",
			before:    "\t\tcheckpointMS = interval.latestCheckpointMS\n",
			after:     "\t\tcheckpointMS = 0 // mutation\n",
			target:    "TestSemanticRecoveryNormalizer/checkpoint_derived_charge_formula_with_checkpoint",
			signature: "valid checkpoint charge: checkpoint-derived charged_duration=95000",
		},
		{
			name:      "checkpoint-plus-grace beyond remaining takes the upper clamp",
			before:    "\t\twant = interval.remainingAtStart\n",
			after:     "\t\twant = interval.remainingAtStart - 1 // mutation\n",
			target:    "TestSemanticRecoveryNormalizer/checkpoint_derived_charge_formula_bounded_by_remaining",
			signature: "bounded-by-remaining charge: checkpoint-derived charged_duration=1000",
		},
		{
