...
func TestMutationApplyConjuncts(t *testing.T) {
	...
	for _, mutation := range []struct {
		...
		},
		{
			...
			source:      "internal/runstore/application.go",
			before:      "\t\tif treeErr == nil && resultTreeErr == nil && afterTree == resultTree {",
			after:       "\t\tif treeErr != nil && resultTreeErr == nil && afterTree == resultTree {",
			packagePath: "./cmd/partitur",
			testName:    "TestApplyRequireVerifiedMaterializesCandidate",
		},
		{
			name:        "completion requires a projected candidate result tree",
			source:      "internal/runstore/application.go",
			before:      "\t\tif treeErr == nil && resultTreeErr == nil && afterTree == resultTree {",
			after:       "\t\tif treeErr == nil && resultTreeErr != nil && afterTree == resultTree {",
			packagePath: "./cmd/partitur",
		...
		},
		{
			...
			source:      "internal/runstore/application.go",
			before:      "err == nil && baseTreeErr == nil && restored != baseTree",
			after:       "err != nil && baseTreeErr == nil && restored != baseTree",
			packagePath: "./cmd/partitur",
			testName:    "TestApplyRollbackRestoresTouchedPathsBeforeReportingUnverifiable",
		},
		{
			name:        "rollback only runs after a projected candidate base tree",
			source:      "internal/runstore/application.go",
			before:      "err == nil && baseTreeErr == nil && restored != baseTree",
			after:       "err == nil && baseTreeErr != nil && restored != baseTree",
			packagePath: "./cmd/partitur",
			// Not the sabotaged patch: that one never attaches, so the base is
			// already in place and a skipped restore changes nothing. Only a patch
			// that landed and then failed verification has work to undo.
			testName: "TestApplyRollbackRestoresTouchedPathsBeforeReportingUnverifiable",
		},
		{
