...
type receiptObserverFunc func(DurabilityReceipt)

func (observer receiptObserverFunc) Observed(receipt DurabilityReceipt) {
	observer(receipt)
}
...
var (
	...
	ErrDecisionResolutionNotAllowed = errors.New("decision resolution requires the expected pending decision on a nonterminal run")
	ErrMissingPinnedSnapshot        = errors.New("missing pinned score snapshot")
	ErrMissingPreparePlan           = errors.New("missing prepare plan")
	ErrPrepareLeaseEpochMismatch    = errors.New("prepare lease epoch mismatch")
	ErrPrepareWaiting               = errors.New("prepare acknowledgement is still pending")
