...
// budgetPrepareCapMS is the fixture score's active_wall_clock_min in integer
// milliseconds.
const budgetPrepareCapMS int64 = 10 * 60 * 1000

// TestBudgetDeadlineWithPendingPrepareIsSettledByTheLiveDriver pins DESIGN §6's
// `budget.own-deadline-quiesce-closes-exhausted-and-defers-the-decision` on the
// state #480 item 3 named: the adapter has reported a prepared auto approval
// (approvalPrepared) and the active budget deadline then fires inside the same
// client.Execute call. The rule the clause fixes is that the live driver closes
// its own adapter interval budget_exhausted/measured because it is the opener
// and exhaustion is what ended the interval; runs neither the mid-flight
// terminal chain, whose attempt.failed the barrier refuses, nor an abandonment,
// whose reasons are the closed set base_head_changed | plan_invalidated |
// cancelled; completes the prepare through §6 steps 2-3 as the responsive
// driver, leaving recovery nothing to complete or abandon; and leaves the
// budget's terminal decision to the between-unit scheduler once the settlement
// has lifted the barrier.
func TestBudgetDeadlineWithPendingPrepareIsSettledByTheLiveDriver(t *testing.T) {
	...
	adapterFixture := &preparingThenDeadlineExecutor{t: t, baseHash: hash}
	result := driver.ExecuteAttempt(context.Background(), driver.AttemptExecution{
		...
		Probe: faultpoint.Nop{}, Client: adapterFixture,
	...
	// The episode ends in the run the between-unit scheduler failed for budget
	// exhaustion, not in the barrier refusal "illegal transition: attempt.failed:
	// prepare_pending" that the unguarded terminal branch produced.
	if result.Outcome != driver.OutcomeFailed || result.Reason != "budget_exhausted" || result.Err != nil {
		t.Fatalf("live result = %+v, want FAILED budget_exhausted", result)
	}
	// The settled approval selects a revision restart, but the spent budget stops
	// the scheduler before any successor attempt runs: the adapter is entered once.
	if adapterFixture.calls != 1 {
		t.Fatalf("adapter Execute calls = %d, want 1 (a later call fails with %v)", adapterFixture.calls, errContinuedAfterSettledPrepare)
	}
	...
	stops := budgetPrepareClosesOf(t, live.Events, adapterInterval)
	if len(stops) != 1 {
		t.Fatalf("live closes of the adapter interval = %d, want 1", len(stops))
	}
	stop := budgetPreparePayload(t, stops[0])
	if stop["reason"] != "budget_exhausted" || stop["charging"] != "measured" {
		t.Fatalf("adapter execution.stopped = %#v, want the opener's measured budget close", stop)
	}
	if _, grace := stop["accounting_grace_ms"]; grace {
		t.Fatalf("adapter execution.stopped = %#v, want a measured close with no clamp audit field", stop)
	}
	...
	// The run's budget exhaustion is not lost. The settlement lifts the barrier;
	// the between-unit scheduler then reads remaining_time == 0 and takes §6's
	// budget path — the live twin of RC-RESUME-045 — which records the exhaustion
	// in movement.failed for the still-RUNNING movement and then in run.failed for
	// the run. Both follow the settlement, and run.failed ends the journal.
	movementFailures := budgetPrepareEventsOfType(live.Events, runstate.EventMovementFailed)
	if len(movementFailures) != 1 {
		t.Fatalf("live movement.failed count = %d, want 1", len(movementFailures))
	}
	movementFailure := budgetPreparePayload(t, movementFailures[0])
	if movementFailure["reason"] != "budget_exhausted" || movementFailure["run_failed"] != false {
		t.Fatalf("movement.failed = %#v, want the between-unit budget failure", movementFailure)
	}
	runFailures := budgetPrepareEventsOfType(live.Events, runstate.EventRunFailed)
	if len(runFailures) != 1 {
		t.Fatalf("live run.failed count = %d, want 1", len(runFailures))
	}
	if runFailure := budgetPreparePayload(t, runFailures[0]); runFailure["reason"] != "budget_exhausted" {
		t.Fatalf("run.failed = %#v, want reason budget_exhausted", runFailure)
	}
	if budgetPrepareIndexOf(live.Events, approvals[0]) >= budgetPrepareIndexOf(live.Events, movementFailures[0]) ||
		budgetPrepareIndexOf(live.Events, movementFailures[0]) >= budgetPrepareIndexOf(live.Events, runFailures[0]) {
		t.Fatal("the budget path does not follow the settlement that lifted the barrier")
	}
	if last := live.Events[len(live.Events)-1]; last.EventID != runFailures[0].EventID {
		t.Fatalf("journal ends with %s, want run.failed {budget_exhausted}", last.Type)
	}
	...
	if state.Projection.State.Run != runstate.RunFailed {
		t.Fatalf("run lifecycle = %s, want FAILED", state.Projection.State.Run)
	}
	if state.Projection.Scheduler.RemainingTime != 0 {
		t.Fatalf("remaining budget = %d, want 0", state.Projection.Scheduler.RemainingTime)
	}
	...
	adapterStops := budgetPrepareClosesOf(t, recovered.Events, adapterInterval)
	if len(adapterStops) != 1 {
		t.Fatalf("recovered closes of the budget-deadline adapter interval = %d, want 1", len(adapterStops))
	}
	recoveredStop := budgetPreparePayload(t, adapterStops[0])
	if recoveredStop["reason"] != "budget_exhausted" || recoveredStop["charging"] != "measured" {
		t.Fatalf("recovered adapter close = %#v, want the live measured budget close", recoveredStop)
	}
}
...
func budgetPrepareClosesOf(t *testing.T, events []runstate.Event, interval string) []runstate.Event {
	t.Helper()
	matched := make([]runstate.Event, 0, 1)
	for _, event := range budgetPrepareEventsOfType(events, runstate.EventExecutionStopped) {
		if budgetPreparePayload(t, event)["interval_id"] == interval {
			matched = append(matched, event)
		}
	}
	return matched
...
func budgetPreparePendingFixture(t *testing.T) (
	*validate.Preparation, *runstore.Store, *runstore.Driver,
	...
	budgetPrepareSeedConsumedBudget(t, started.RunID, authority)
	...
	if input.Projection.Scheduler.RemainingTime != budgetPrepareRemainingMS {
		t.Fatalf("seeded remaining budget = %d, want %d", input.Projection.Scheduler.RemainingTime, budgetPrepareRemainingMS)
	}
...
// budgetPrepareSeedConsumedBudget spends all but budgetPrepareRemainingMS of the
// score's active budget in one already-closed composition interval, so the
// remaining budget the projection reports is the remaining budget the attempt
// below is given. Without it the attempt's deadline and the run's accounting
// would disagree and the scheduler would find budget left after the settlement.
func budgetPrepareSeedConsumedBudget(t *testing.T, runID runstate.RunID, authority *runstore.Driver) {
	t.Helper()
	interval, err := workspace.NewID()
	if err != nil {
		t.Fatal(err)
	}
	for _, seed := range []struct {
		...
		{runstate.EventExecutionStarted, map[string]any{
			"interval_id": interval, "phase": "composition",
			"wall_start": time.Now().UTC().Format("2006-01-02T15:04:05.000Z"), "remaining_at_start": budgetPrepareCapMS,
		}},
		{runstate.EventExecutionStopped, map[string]any{
			"interval_id": interval, "reason": "normal", "charging": "measured",
			"charged_duration": budgetPrepareCapMS - budgetPrepareRemainingMS,
		}},
	} {
		payload, err := json.Marshal(seed.payload)
		if err != nil {
			t.Fatal(err)
		}
		if _, err := authority.Append(runstate.Event{
			RunID: runID, ScoreRevision: 1, Type: seed.eventType, Payload: payload,
		}, faultpoint.ReceiptAddress("test.budget_prepare.seed."+string(seed.eventType))); err != nil {
			t.Fatal(err)
		}
	}
}
