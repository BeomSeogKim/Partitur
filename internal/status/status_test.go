...
func TestReadScanSkipsUnreadableNonSelectedRun(t *testing.T) {
	root, runID := statusFixture(t)
	writeLegacyUnreadableRun(t, root, runID)

	report, err := ReadObservation(root, "")
	if err != nil {
		t.Fatal(err)
	}
	if report.Run.ID != runID || report.Run.Lifecycle != string(runstate.RunRunning) {
		t.Fatalf("run = %+v, want %s in RUNNING", report.Run, runID)
	}
}

func TestReadSelectedUnreadableRunStillReportsJournalCorrupt(t *testing.T) {
	root, runID := statusFixture(t)
	legacyID := writeLegacyUnreadableRun(t, root, runID)

	_, err := Read(root, legacyID)
	if !errors.Is(err, runstore.ErrJournalCorrupt) || !strings.Contains(err.Error(), `unknown field "observed_at"`) {
		t.Fatalf("error = %v, want journal corrupt caused by legacy observed_at", err)
	}
}

func TestReadFailClosedScanStillReportsUnreadableDiscoveredRun(t *testing.T) {
	root, runID := statusFixture(t)
	writeLegacyUnreadableRun(t, root, runID)
...
func TestReadScanStillRefusesRunWithUnreadableSnapshot(t *testing.T) {
	root, _ := statusFixture(t)
	store, err := runstore.New(root, faultpoint.Nop{})
	if err != nil {
		t.Fatal(err)
	}
	err = store.Mutate("missing-snapshot", "", func(transaction *runstore.Txn) error {
		payload, marshalErr := json.Marshal(runStartedPayload(t, statusScore()))
		if marshalErr != nil {
			return marshalErr
		}
		_, appendErr := transaction.At("status.missing-snapshot").Append(runstate.Event{
			RunID: "missing-snapshot", ScoreRevision: 1, Type: runstate.EventRunStarted, Payload: payload,
		})
		return appendErr
	})
	if err != nil {
	...
	_, err = ReadObservation(root, "")
	if !errors.Is(err, ErrSnapshot) {
		t.Fatalf("error = %v, want snapshot error", err)
	}
}

func writeLegacyUnreadableRun(t *testing.T, root, readableID string) string {
	t.Helper()
	const legacyID = "legacy-run"
	readableJournal, err := os.ReadFile(filepath.Join(root, ".partitur", "runs", readableID, "journal.jsonl"))
	if err != nil {
		t.Fatal(err)
	}
