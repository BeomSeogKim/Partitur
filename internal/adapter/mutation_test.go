...
func TestMutationCodexAttemptEnvironmentMustReachInvocation(t *testing.T) {
	environment, err := mutationtest.SnapshotGoEnvironment()
	if err != nil {
		t.Fatal(err)
	}
	...
	source := filepath.Join(copyRoot, "internal", "adapters", "codex", "adapter.go")
	contents, err := os.ReadFile(source)
	if err != nil {
		t.Fatal(err)
	}
	const before = "Env:    environment,"
	const after = "Env:    os.Environ(),"
	if count := strings.Count(string(contents), before); count != 1 {
		t.Fatalf("mutation anchor count=%d, want 1", count)
	}
	mutated := strings.Replace(string(contents), before, after, 1)
	...
	result := mutationtest.Run(ctx, mutationtest.Child{
		Dir:         copyRoot,
		Package:     "./internal/adapters/codex",
		TestPattern: "TestExecuteAgainstHelperCLIScratchEnvironment",
		TestNames:   []string{"TestExecuteAgainstHelperCLIScratchEnvironment"},
		Environment: environment.ChildEnvironment(os.Environ(), "PARTITUR_MUTATION_CHILD=1"),
	})
	if result.Outcome != mutationtest.Killed {
		t.Fatalf("codex attempt environment mutation: %s", result.Diagnostic())
	}
}
