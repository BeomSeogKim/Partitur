...
| W18 integer remaining budget | `real:claude`, `real:codex` execute `remaining_ms` through the strict wire and render the same integer in the performer brief | old `active_wall_clock_min`, fractional, negative, negative-zero, and out-of-safe-range values | Restore the old field/type, weaken the safe-integer or negative-zero guard, or convert the brief back to minutes → `TestServeExecuteRoundTrip`, `TestServeExecuteBudgetIngress`, or `TestRenderPromptIncludesContractAndBrief` |
