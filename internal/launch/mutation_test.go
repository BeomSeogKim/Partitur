...
func TestMutationNilStderrCaptureIsRequired(t *testing.T) {
	...
	if !ok {
		t.Fatal("resolve launch mutation source directory")
	}
	copyRoot := filepath.Join(t.TempDir(), "partitur-mutation-copy")
	repository := filepath.Clean(filepath.Join(filepath.Dir(current), "..", ".."))
	if err := copyLaunchMutationRepository(copyRoot, repository); err != nil {
		t.Fatal(err)
	}
	source := filepath.Join(copyRoot, "internal", "launch", "launch_unix.go")
	contents, err := os.ReadFile(source)
	if err != nil {
		t.Fatal(err)
	}
	const before = "command.Stderr = stderrCapture.writer"
	const after = "_ = stderrCapture\n\tcommand.Stderr = request.Stderr"
	...
	result := mutationtest.Run(ctx, mutationtest.Child{
		Dir:         copyRoot,
		Package:     "./internal/launch",
		TestPattern: "TestLaunchFailureWithoutStderrIncludesSanitizedTrampolineDiagnostic",
		TestNames:   []string{"TestLaunchFailureWithoutStderrIncludesSanitizedTrampolineDiagnostic"},
		Environment: environment.ChildEnvironment(os.Environ(), "PARTITUR_MUTATION_CHILD=1"),
	})
	if result.Outcome != mutationtest.Killed {
		t.Fatalf("nil stderr capture mutation: %s", result.Diagnostic())
	}
}

func TestMutationPostReleaseFailureMustNotWrapStderr(t *testing.T) {
	environment, err := mutationtest.SnapshotGoEnvironment()
	if err != nil {
		t.Fatal(err)
	}
	...
	const before = "return nil, releasedError(fmt.Errorf(\"close launch gate: %w\", err))"
	const after = "_ = releasedError\n\t\treturn nil, launchError(fmt.Errorf(\"close launch gate: %w\", err))"
	if count := strings.Count(string(contents), before); count != 1 {
		t.Fatalf("mutation anchor count=%d, want 1", count)
	}
	mutated := strings.Replace(string(contents), before, after, 1)
	...
	result := mutationtest.Run(ctx, mutationtest.Child{
		Dir:         copyRoot,
		Package:     "./internal/launch",
		TestPattern: "TestPostReleaseFailureDoesNotWaitForDescendantStderr",
		TestNames:   []string{"TestPostReleaseFailureDoesNotWaitForDescendantStderr"},
		Environment: environment.ChildEnvironment(os.Environ(), "PARTITUR_MUTATION_CHILD=1"),
	})
	if result.Outcome != mutationtest.Killed {
		t.Fatalf("post-release stderr wrap mutation: %s", result.Diagnostic())
	}
}
